...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        if conv_stem:
            ...
            #     img_size=img_size, patch_size=(1,2,2), in_chans=1, embed_dim=embed_dim[0])
            ...
            # ruishaohao modify for keeping D dim 2025/4/1
            self.patch_embed1 = PatchEmbed(
            ...
            self.patch_embed3 = PatchEmbed(
                img_size=img_size // 8, patch_size=(1,2,2), in_chans=embed_dim[1], embed_dim=embed_dim[2])      ###wll 20230711 modify stride
            self.patch_embed4 = PatchEmbed(
    ...
    def forward_features(self, x):
        ...
        t = x.shape[1]
        x = x.permute(0,1,4,2,3)      #### change C*H*W*D to C*D*H*W # 希望 D 维度不进行下采样
        if t==1:
            x1 = self.patch_embed11(x)
        else:
...
if __name__=="__main__":
    uniformer = uniformer_small(in_chans=3).cuda()
    data = torch.randn(2, 3, 224, 224,13).cuda() # C*H*W*D
    out = uniformer(data)